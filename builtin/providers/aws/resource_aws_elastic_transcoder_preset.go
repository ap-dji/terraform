				...
				Elem: &schema.Resource{
					...
					Schema: map[string]*schema.Schema{
						...
						"codec_options": &schema.Schema{
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									// CodecOptions is a map[string]*string in the API
									// but we can't nest a schema.TypeMap under a set.Schema.
									"profile": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									...
									"max_reference_frames": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									},
									"max_bit_rate": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									},
									...
									"interlace_mode": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									},
									"color_space_conversion_mode": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									},
									"chroma_subsampling": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									},
									"loop_count": &schema.Schema{
										Type:     schema.TypeString,
										Optional: true,
										ForceNew: true,
									},
...
func exapandETVideoParams(d *schema.ResourceData) *elastictranscoder.VideoParameters {
	...
	return &elastictranscoder.VideoParameters{
		...
		CodecOptions:       expandETVideoCodecOptions(p["codec_options"].(*schema.Set)),
...
func expandETVideoCodecOptions(s *schema.Set) map[string]*string {
	if s == nil || s.Len() == 0 {
		return nil
	}
	...
	if p := getStringPtr(m, "profile"); p != nil {
		codecOpts["Profile"] = p
	}
	if p := getStringPtr(m, "level"); p != nil {
		codecOpts["Level"] = p
	}
	if p := getStringPtr(m, "max_reference_frames"); p != nil {
		codecOpts["MaxReferenceFrames"] = p
	}
	if p := getStringPtr(m, "max_bit_rate"); p != nil {
		codecOpts["MaxBitRate"] = p
	}
	if p := getStringPtr(m, "buffer_size"); p != nil {
		codecOpts["BufferSize"] = p
	}
	if p := getStringPtr(m, "interlace_mode"); p != nil {
		codecOpts["InterlaceMode"] = p
	}
	if p := getStringPtr(m, "color_space_conversion_mode"); p != nil {
		codecOpts["ColorSpaceConversionMode"] = p
	}
	if p := getStringPtr(m, "chroma_subsampling"); p != nil {
		codecOpts["ChromaSubsampling"] = p
	}
	if p := getStringPtr(m, "loop_count"); p != nil {
		codecOpts["LoopCount"] = p
	}
...
func flattenETVideoParams(video *elastictranscoder.VideoParameters) []map[string]interface{} {
	...
	m.Set("codec_options", flattenETVideoCodecOptions(video.CodecOptions))
